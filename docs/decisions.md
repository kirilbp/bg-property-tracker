...
**Breakpoint decision**: `index.html` has no single existing "mobile nav" breakpoint to copy - the closest real conventions are the two-column-to-single-column stacking points already used for map/chart-style layouts (`.btl-grid` at `max-width: 700px`, `.detail-grid` at `max-width: 800px`) plus a separate, unrelated set of card-grid reflow points (560/900/1400px) and a fluid `auto-fit`/`minmax` grid for `.detail-history-row` (the actual item-7/8 price-chart-and-map pairing) that uses no fixed breakpoint at all. Picked **768px**, the midpoint of the 700/800 pair and the de facto industry-standard tablet/mobile split, as the closest reasonable fit rather than inventing an unrelated number - documented directly in the CSS comment so a future builder doesn't have to re-derive this reasoning.
