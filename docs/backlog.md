...
**1. "Similar listings" strip (listing detail page).** `findSimilarListings(merged)` is a plain heuristic over the already-loaded `MERGED_LISTINGS` array (no ML, no new fetch): same `city_key`, same `typeFilterBucket()`, price within ±20%, excludes the current listing and anything `status === 'sold'`, sorted by closeness in price, capped at 6. Mirrors the existing `areaDataMatches()` function's own city+bucket filtering approach (Area Data tab) minus its area-key AND (similar listings can be anywhere in the same city - area alone is often too narrow to return 4-6 results) with a price band substituted in. Rendered via the same `createListingCard()` DOM-node component the main results grid/Dashboard/Comparables all already use (not a second parallel card template), in a new horizontal-scroll `.similar-listings-strip` section below the whole `detail-grid` (photo + info/tabs column), so it reads as "below the main listing content" per the task, not squeezed into the existing tab system. Section header styled to match `.card h2` (serif, medium weight) - the same section-header treatment used everywhere else on the site.

**2. Saved filter presets.** A deliberately lighter-weight sibling to Lead Generators, not a variant of it: `localStorage` key `filterPresets` (own array, own load/save/delete trio, same no-login pattern as `sitePreferences`/`leadGenerators`), a "Saved filters" row directly under the existing filter panel on the Leads page with a compact pill per saved preset (name only - a hover title shows a short human-readable summary, e.g. "Sofia · Flats · €80,000–€250,000 · 2 rooms", per design-guidelines.md's "small set visible, density one interaction deeper" principle) plus a "+ Save current filters" affordance that reveals a one-field inline name form (no modal - Lead Generators already owns the modal-based "save a full config" pattern; this stays visibly simpler). A saved preset snapshots exactly what `readFilterState()` already reads off the filter panel/tiles (min/max price, min sqm, area, rooms, min days, min reduced, exclude-sold, only-with-photos, sort, search text, city/type/oblast tile filters) minus `activeLeadGenerator`, since a preset is a plain filter combination, independent of the separate Lead Generator feature. Loading a preset (`applyFilterPreset()`) restores every dimension at once, including city+type+oblast together - this differs from clicking a city/type/oblast tile elsewhere (each of which clears the other two as a single-filter UX shortcut, see `applyCityFilter()`/`applyTypeFilter()`/`applyOblastFilter()`), but `matchesAllFilters()` already ANDs all of them together regardless of how each was set, so restoring a preset's full combination is a real, valid filter state, not a workaround.

**3. Map marker clustering.** Checked every map in the app for the "does a national/zoomed-out view get overwhelming" question the task posed, rather than assuming: the Comparables map (`renderCmpMapView()`) is the one real case - an empty city/area picker matches every tracked listing nationwide (`runComparablesSearch()`'s own `inExactArea` logic), and the map plotted up to 300 raw `L.circleMarker` dots at national zoom with no grouping. Every other map in the app is already either naturally scoped to a small set (Lead Generator radius/polygon preview, Pipeline map, per-listing detail Comparables/radius maps) or already pre-aggregates by area rather than plotting one marker per listing (the Market Data hub's Live Map bubbles one circle per area via `marketAggregateRows()`, sized/shaded by metric - already a form of clustering). **Chose Leaflet.markercluster (vendored via a CDN `<script>`/`<link>` tag, the same pattern this codebase already uses for Leaflet/Leaflet.draw/Chart.js/Supabase-js - not an npm/bundler dependency) over a hand-rolled grid-bucket approach**: it's the well-known, purpose-built solution for exactly this problem (spiderfy, expand-on-zoom, and cluster-radius handling all come for free instead of being re-implemented and re-tested from scratch), and loading a third-party library via a CDN tag is this codebase's existing dependency pattern, not a new one. Only the base `MarkerCluster.css` is loaded - `MarkerCluster.Default.css` (the library's own green/yellow/orange size-tiered cluster theme) is deliberately NOT loaded, since that multi-color scale would violate design-guidelines.md's one-accent-color rule; every cluster instead renders through a custom `iconCreateFunction` (`brassClusterIcon()`) as a single brass circle at every size tier, only the diameter and the count inside it changing. Individual (unclustered) listing markers switched from `L.circleMarker` to a small brass `L.divIcon` (`brassDotIcon()`) because `MarkerClusterGroup` calls `setIcon()`/`getIcon()` on its children for spiderfying, which `CircleMarker` doesn't implement - visually near-identical at this size, so this isn't a visible style change. The old 300-marker cap (there specifically to keep the unclustered case usable) is raised to 2,000 now that clustering removes the need for as tight a limit, while staying bounded rather than unbounded.

**4. Dashboard "what changed" digest card.** New card ("What's changed") pinned near the top of the Dashboard, directly under Start Here. Diffs the current state of every saved-or-pipelined listing (`dashboardTrackedListingIds()` - the union of the two tracking sets the app already maintains, not a new one) against a `localStorage`-persisted snapshot (`dashboardDigestSnapshot`: `{ visitedAt, listings: { [id]: { price_eur, status, title } } }`) taken the last time the Dashboard was actually visited, surfacing price drops (sage "PRICE DROP" label + old→new price), status changes (ink "SOLD"/"REMOVED" label - "removed" specifically covers a listing that's dropped out of `MERGED_LISTINGS` entirely, i.e. delisted across every tracked portal, not just marked sold), and new Lead Generator matches (reuses each generator's own already-computed `fresh` count from `computeLeadGenCounts()`, summed - one real signal, not a second parallel "new" tracker with its own possibly-disagreeing definition). **Judgment call on when the baseline advances**: `markDashboardVisited()` (which overwrites the stored snapshot) is called only from `showSection()`'s `'dashboard'` branch - i.e. on an actual navigation into the Dashboard tab - never from the many other places `renderDashboard()` itself gets called (data reload, toggling a save, checking a lead generator), so a diff the user hasn't looked at yet can't get silently erased by an incidental re-render before they see it. Guarded on `MERGED_LISTINGS.length > 0` so an early nav-click before the bulk fetch resolves can't snapshot an empty array as "everything removed". First-ever visit (no stored snapshot yet) shows a neutral "this is your first visit" message rather than a false "nothing changed".

**Verification**: real Playwright checks (Chromium, headless) against the actual worktree `index.html`, reusing the established fixture-backed harness pattern from prior sessions' `verify_dealcalc.js`/`testharness/serve_and_shot.js` (a local static server + `page.route()` intercepts serving vendored Leaflet/Leaflet.draw/Leaflet.markercluster/Chart.js/Supabase-js locally and fulfilling the Supabase REST endpoints from a committed 304-row fixture dataset, since this sandbox's egress proxy blocks the real CDNs/API). All checks passed at both 1440px and 390px:
- Similar listings: for a real fixture Sofia flat, the rendered strip's 6 cards exactly matched an independently re-derived (not copied from `index.html`) same-city/same-type/±20%-price computation, excluded the subject listing itself, and rendered at 390px.
- Filter presets: saved a preset with a specific combination (price range, min sqm, rooms, sort, search text, only-with-photos), confirmed the exact values landed in `localStorage`, reloaded the page fresh (simulating a return visit), cleared the fields, loaded the preset back, and confirmed every field round-tripped exactly; delete round-tripped too.
- Map clustering: a national (no-city) Comparables search returned 38 located listings; the initial (fitBounds/zoomed-out) view grouped them into 5 cluster icons + 1 individual marker (6 visible DOM nodes vs. 38 raw listings - a real reduction, not just present-but-inert clustering code), zooming to z18 expanded every cluster into individual markers, and no `marker-cluster-small/medium/large` (the library's own default multi-color theme classes) were present anywhere.
- Dashboard digest: seeded a fake "week-ago" snapshot for two real fixture listings (one with a higher price than its current fixture price, one marked available where the fixture now has it sold), confirmed the rendered digest showed the exact correct price-drop delta and a "Sold" row, confirmed visiting the Dashboard advanced the stored snapshot, and confirmed a second render in the same session correctly showed "Nothing new since your last visit" (baseline-consumed behavior working as designed).
